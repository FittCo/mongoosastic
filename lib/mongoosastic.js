...
function createEsClient (options) {
  console.log('creating new client w/ options', options)
  let esOptions = {};

  if (options) {
    esOptions.node = options.node ? options.node : 'http://localhost:9200'
    if (options.auth) esOptions.auth = options.auth
    if (options.maxRetries) esOptions.maxRetries = options.maxRetries;
    if (options.requestTimeout) esOptions.requestTimeout = options.requestTimeout
  }
