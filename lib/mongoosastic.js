...
function createEsClient (options) {
  ...
  return new elasticsearch.Client({
    node: options && options.node ? options.node : 'http://localhost:9200',
    auth: options && options.auth ? options.auth : null,
    maxRetries: options && options.maxRetries ? options.maxRetries : null,
    requestTimeout: options && options.requestTimeout ? options.requestTimeout : null,
  })
}
