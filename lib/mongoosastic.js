...
// todo: add node/auth options as a backup
console.log('new client options', options)
  ...
  //   }
  // }
  esOptions.node = options && options.node ? options.node : 'http://localhost:9200'
  esOptions.auth = options && options.auth ? options.auth : null
...
function Mongoosastic (schema, pluginOpts) {
  ...
  // todo: figure out what the issue here.... why is it not
  // todo: using the client were passing it here???

  console.log('initializing with these options...', options)
  console.log('options.esClient???', options.esClient)
